...
The above summary of the scaled dataset shows that this scaling evened out the variability of some of the variables, particularly the crime rate.
...
Let's perform the K-means clustering with Euclidian distances and K=2 clusters
...
The interpretation the the twcss number is, that the optimal number of clusters is when the twcss drops radically. There, it obviously drops radically when K changes from 1 to 2. Also 2 to 3 might be interpreted as a radical drop (it's about 25% of twcss). Drop from 3 to 4 is not radical anymore.Therefore we'll stick to K=3 as the optimal.
